...
-- 计算逻辑：先计算原始price，然后除以0.000002向上取整，再乘回0.000002，即
...
data AS (
    SELECT 
        ...
        -- 计算价格：先计算原始价格，再按最小计量0.000002向上取整
        ...
        printf("%12.6f", CEIL((SUM(
            CASE 
                ...
                ELSE CEIL(l.prompt_tokens * JSON_EXTRACT(l.metadata, '$.input_ratio') + l.completion_tokens * JSON_EXTRACT(l.metadata, '$.output_ratio'))
            END
        ) * 0.002/1000) / 0.000002) * 0.000002 - CEIL((SUM(l.quota) * 0.002/1000) / 0.000002) * 0.000002) AS "Price Diff (USD)"
    FROM 
        logs l
    JOIN 
